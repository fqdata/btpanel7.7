...
if [ "${publicFileMd5}" != "${md5check}"  ]; then
	wget -O Tpublic.sh https://gcore.jsdelivr.net/gh/fqdata/btpanel7.7/install/public.sh -T 20;
...
setup_path=/www
version=$(curl -Ss --connect-timeout 5 -m 2 https://gcore.jsdelivr.net/gh/fqdata/btpanel7.7/api/panel/get_version)
if [ "$version" = '' ];then
	version='7.7.0'
fi
...
wget -T 5 -O /tmp/panel.zip https://gcore.jsdelivr.net/gh/fqdata/btpanel7.7/install/src/LinuxPanel-7.7.0.zip
...
if [ "${check_bt}" = "" ];then
	rm -f /etc/init.d/bt
	wget -O /etc/init.d/bt https://gcore.jsdelivr.net/gh/fqdata/btpanel7.7/install/src/bt6.init -T 20
